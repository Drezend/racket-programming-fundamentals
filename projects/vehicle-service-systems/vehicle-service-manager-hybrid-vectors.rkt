...
; Product representation as list in vector:
; list index 0: code (Unique numeric identifier for the product)
; list index 1: name (Descriptive name of the product)
; list index 2: value (Unit price of the product)

; Service voucher representation as list in vector:
; list index 0: plate (Vehicle license plate)
; list index 1: date (Service date in YYYY-MM-DD format)
; list index 2: clientName (Name of the customer)
; list index 3: nit (Tax identification number)
; list index 4: codes (Vector of product codes included in the service)
; list index 5: quantities (Vector of quantities for each product)

;; ========================== INITIAL STATE =========================

; Initial empty product portfolio (now a vector instead of list)
(define initialPortfolio (vector))

; Initial empty voucher list (now a vector instead of list)
...
(define (CheckCodeExistsAux2 productCode productPortfolio lowerBound upperBound middlePoint)
  ...
  (define middleCode (list-ref (vector-ref productPortfolio middlePoint) 0))
...
(define (FindProduct productCode productPortfolio lowerBound upperBound)
  ...
  ; Returns: Product list or "Not found" product
  (if (>= lowerBound upperBound)
      (list 0 "No encontrado" 0)
...
(define (FindProductAux2 productCode productPortfolio lowerBound upperBound middlePoint)
  ...
  (define middleCode (list-ref middleProduct 0))
  (if (= productCode middleCode)
      middleProduct
...
(define (InsertInOrder newProduct productPortfolio)
  ; newProduct: Product list to insert
...
(define (InsertInOrderAux2 newProduct productPortfolio currentIndex resultVector)
  ...
  (if (< (list-ref newProduct 0) (list-ref (vector-ref productPortfolio currentIndex) 0))
...
(define (CalculateSubtotalItem productPortfolio productCodes productQuantities currentIndex accumulatedTotal)
  ...
  (define unitPrice (list-ref currentProduct 2))
...
(define (PrintCurrentInvoiceItem productPortfolio productCodes productQuantities currentIndex)
  ...
  (printf "│           │   ~a│ ~a│      ~a│ ~a││\n"
          ...
          (string-pad-right (~a (list-ref currentProduct 1)) 48 #\space)
...
(define (DisplayVoucherInfo plate date clientName nit codes quantities voucherNumber productPortfolio)
  ...
  ; nit: Tax ID
  ; codes: Vector of product codes
  ; quantities: Vector of product quantities
...
(define (DisplayVoucherProductsAux productPortfolio codes quantities currentIndex totalProducts)
  ...
  (printf "│ │ ~a│ ~a│ ~a││\n"
          ...
          (string-pad-right(~a (list-ref product 1)) 48 #\space)
...
(define (DisplayProductRow productPortfolio currentIndex totalProducts)
  ...
  (printf "│   ~a│ ~a│ ~a│\n"
          (string-pad-right (~a (list-ref currentProduct 0)) 4 #\space)
          (string-pad-right (~a (list-ref currentProduct 1)) 48 #\space)
          (string-pad-right (~a (FormatNumberWithCommas (list-ref currentProduct 2))) 14 #\space)
  )
...
(define (DisplayVoucherItems voucher voucherNumber)
  ...
  (DisplayVoucherItemsAux (list-ref voucher 4) 
                         (list-ref voucher 5) 
                         0 
                         (vector-length (list-ref voucher 4))
                         (list-ref voucher 0)
                         (list-ref voucher 1)
                         (list-ref voucher 2)
                         (list-ref voucher 3)
...
(define (FinalizeNewVoucher productPortfolio voucherList vehiclePlate serviceDate clientName taxId productCodes productQuantities)
  ...
  (define newVoucher (list vehiclePlate serviceDate clientName taxId productCodes productQuantities))
  (define updatedVoucherList (vector-append voucherList (vector newVoucher)))
  (printf "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n")
  (printf "┌───────────────────────────────────────────────────────────┐\n")
  (printf "│ Comprobante generado exitosamente                         │\n")
  (printf "└───────────────────────────────────────────────────────────┘\n")
  (DisplayVoucherInfo (list-ref newVoucher 0) (list-ref newVoucher 1) (list-ref newVoucher 2) (list-ref newVoucher 3) (list-ref newVoucher 4) (list-ref newVoucher 5) (vector-length updatedVoucherList) productPortfolio)
...
(define (FinalizeExistingVoucher productPortfolio voucherList vehiclePlate serviceDate clientName nit productCodes productQuantities voucherIndex)
  ...
  ; Returns: Nothing (proceeds to main menu)
  (define updatedVoucher (list vehiclePlate serviceDate clientName nit productCodes productQuantities))
...
(define (DisplayExistingProduct productPortfolio voucherList newCode)
  ...
  (printf "│ Nombre: ~a~a│\n" (list-ref existingProduct 1) (make-string (- 64 (string-length (list-ref existingProduct 1))) #\space))
  (printf "│ Valor: ~a~a│\n" (list-ref existingProduct 2) (make-string (- 65 (string-length (number->string (list-ref existingProduct 2)))) #\space))
  (AddProductLoop productPortfolio voucherList))
...
(define (AddNewProduct productPortfolio voucherList newCode)
  ...
  (define newProduct (list newCode newName newValue))
...
(define (AddToExistingVoucherAux productPortfolio voucherList voucherNumber)
  ...
  (GetVoucherProducts productPortfolio voucherList 
                      (list-ref selectedVoucher 0)
                      (list-ref selectedVoucher 1)
                      (list-ref selectedVoucher 2)
                      (list-ref selectedVoucher 3)
                      (list-ref selectedVoucher 4)
                      (list-ref selectedVoucher 5)
                      #f
...
(define (PrintInvoice productPortfolio voucherList index)
  ...
  (define vehiclePlate (list-ref currentVoucher 0))
  (define serviceDate (list-ref currentVoucher 1))
  (define clientName (list-ref currentVoucher 2))
  (define taxId (list-ref currentVoucher 3))
  (define productCodes (list-ref currentVoucher 4))
  (define productQuantities (list-ref currentVoucher 5))
