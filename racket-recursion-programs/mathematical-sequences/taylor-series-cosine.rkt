...
Program description: This program calculates the Taylor series expansion for the function cos(x).
...
( define ( ComputeTaylorSeries xValue numberOfTerms currentTerm totalSum )
  ...
  ( if ( < currentTerm numberOfTerms )
      ( begin
        ( printf "Término #~a: ~a\n" currentTerm ( * ( expt -1 currentTerm ) ( / ( expt xValue ( * 2 currentTerm ) ) ( CalculateFactorial ( * 2 currentTerm ) ) ) ) )
        ( ComputeTaylorSeries xValue numberOfTerms ( + currentTerm 1 ) ( + totalSum ( * ( expt -1 currentTerm ) ( / ( expt xValue ( * 2 currentTerm ) ) ( CalculateFactorial ( * 2 currentTerm )  ) 1.0 ) ) ) )
      ); end begin
  ; else
      ( printf "\nTotal suma: ~a\n" totalSum )
  ); end if
