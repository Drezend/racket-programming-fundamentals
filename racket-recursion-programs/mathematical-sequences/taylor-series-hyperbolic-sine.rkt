...
Program description: This program calculates the Taylor series expansion for the function sinh(x).
...
( define ( ComputeTaylorSeries xValue numberOfTerms currentTerm totalSum )
  ...
  ( if ( < currentTerm numberOfTerms )
      ( begin
        ( printf "Término #~a: ~a\n" currentTerm ( / ( expt xValue ( + ( * 2 currentTerm ) 1 ) ) ( CalculateFactorial ( + ( * 2 currentTerm ) 1 ) ) ) )
        ( ComputeTaylorSeries xValue numberOfTerms ( + currentTerm 1 ) ( + totalSum ( / ( expt xValue ( + ( * 2 currentTerm ) 1 ) ) ( CalculateFactorial ( + ( * 2 currentTerm ) 1 ) ) 1.0 ) ) )
      ); end begin
  ; else
      ( printf "\nTotal suma: ~a\n" totalSum )
  ); end if
