...
Program description: This program calculates the Taylor series expansion for the function e^x.
...
( define ( ComputeTaylorSeries xValue numberOfTerms currentTerm totalSum )
  ...
  ( if ( < currentTerm numberOfTerms )
      ( begin
        ( printf "Término #~a: ~a\n" currentTerm ( / ( expt xValue currentTerm ) ( CalculateFactorial currentTerm ) ))
        ( ComputeTaylorSeries xValue numberOfTerms ( + currentTerm 1 ) ( + totalSum ( / ( expt xValue currentTerm ) ( CalculateFactorial currentTerm ) 1.0 ) ) )
      ); end begin
  ; else
      ( printf "\nTotal suma: ~a\n" totalSum )
  ); end if
